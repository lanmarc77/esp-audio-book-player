...
typedef enum
{
    ROTARY_ENCODER_SWITCH_UNKOWN = 0,
    ROTARY_ENCODER_SWITCH_SHORT_CLICK,
    ROTARY_ENCODER_SWITCH_LONG_CLICK,
} rotary_encoder_switch_t;
...
typedef struct
{
    ...
    rotary_encoder_switch_t switch_state;
...
typedef struct
{
    ...
    gpio_num_t pin_sw;                      ///< GPIO for Signal Switch from the rotary encoder device
 ...
 * @param[in] pin_sw GPIO number for rotary encoder switch output.
