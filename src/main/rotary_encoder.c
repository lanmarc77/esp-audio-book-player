...
static uint8_t _process(rotary_encoder_info_t * info)
{
    ...
    if (info != NULL)
    {
        ...
        ESP_EARLY_LOGI(TAG, "BA %d%d, state 0x%02x, new state 0x%02x, event 0x%02x",
...
void rotary_encoder_debounce_switch(void * args){
    ...
    uint8_t pressedHold=0;
    uint8_t releasedHold=0;
    int8_t stable=-1;
    uint8_t clickState=0;//0=no click emitted before, 1=click emitted before
    rotary_encoder_info_t * info = (rotary_encoder_info_t *)args;
    while(1){
        vTaskDelay(pdMS_TO_TICKS(10));
        if(!gpio_get_level(info->pin_sw)){
            nowState=0;
        }else{
            nowState=1;
        }
        if(lastButtonState<0){//set the initial state without triggering any events
            lastButtonState=nowState;
        }else{
            if(lastButtonState!=nowState){//change
                pressedHold=0;
                releasedHold=0;
                stable=0;
                lastButtonState=nowState;
                //ESP_LOGI(TAG, "DEBOUNCE: CHANGE");
            }else{
            ...
            if(stable>=2){
                if(lastButtonState==1){//button pressed
                    if(pressedHold==75){//hold long enough trigger long click but only once
                        ...
                        pressedHold=255;
                        clickState=1;//this makes sure when released not short click will be emitted
                        rotary_encoder_event_t queue_event =
                        {   .state =
                            {
                                .direction = ROTARY_ENCODER_SWITCH,
                                .switch_state = ROTARY_ENCODER_SWITCH_LONG_CLICK,
                            },
                        };
                        xQueueSend(info->queue, &queue_event,1);
                    }else{
                        if(pressedHold<255) pressedHold+=1;
                    }
                }else if(lastButtonState==0){
                    if(releasedHold==5){//released long enough trigger short click but only once
                        if(clickState){//ignore the release of a long click
                            clickState=0;
                        }else{
                            //ESP_LOGI(TAG, "DEBOUNCE: SHORT CLICK");
                            rotary_encoder_event_t queue_event =
                            {   .state =
                                {
                                    .direction = ROTARY_ENCODER_SWITCH,
                                    .switch_state = ROTARY_ENCODER_SWITCH_SHORT_CLICK,
                                },
                            };
                        ...
                        if(releasedHold<255) releasedHold+=1;
                    }
                }
            }
        }
...
esp_err_t rotary_encoder_init(rotary_encoder_info_t * info, gpio_num_t pin_a, gpio_num_t pin_b, gpio_num_t pin_sw)
{
    ...
    if (info)
    {
        ...
        info->state.switch_state = ROTARY_ENCODER_SWITCH_UNKOWN;
        ...
        gpio_set_pull_mode(info->pin_b, GPIO_PULLDOWN_ONLY);
        ...
        esp_rom_gpio_pad_select_gpio(info->pin_sw);
        gpio_set_pull_mode(info->pin_sw, GPIO_PULLDOWN_ONLY);
        gpio_set_direction(info->pin_sw, GPIO_MODE_INPUT);

        info->table_state=info->table[info->table_state & 0xf][(gpio_get_level(info->pin_b) << 1) | gpio_get_level(info->pin_a)];

        xTaskCreate(rotary_encoder_debounce_switch, "rotary_encoder_debounce_switch", 1024 * 2, info, uxTaskPriorityGet(NULL), NULL);
